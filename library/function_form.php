...
function buat_textbox($label, $nama, $nilai, $lebar='4', $required = false, $tipe="text",  $placeholder = '', $autocomplete = 'on'){
	echo'<div class="form-group" id="'.$nama.'">
			...
			<div class="col-sm-'.$lebar.'">
			  <input autocomplete="'.$autocomplete.'" type="'.$tipe.'" class="form-control" name="'.$nama.'" value="'.$nilai.'" placeholder="'.$placeholder.'" '.($required ? 'required' : '').' oninvalid="setCustomValidity(\'Mohon Diisi.\')" oninput="setCustomValidity(\'\')">
			</div>
		 </div>';
}
...
function buat_textarea($label, $nama, $nilai, $class='', $required = false){
	echo'<div class="form-group" id="'.$nama.'">
			<label for="'.$nama.'" class="col-sm-2 control-label">'.$label.'</label>
			<div class="col-sm-10">
			  <textarea class="form-control '.$class.'" rows="8" name="'.$nama.'" '.($required ? 'required' : '').'>'.$nilai.'</textarea>
			</div>
		 </div>';
}
