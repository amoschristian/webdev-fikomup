...
switch ($show) {
    ...
    default:
        ...
        if ($_SESSION['leveluser'] == "admin") $query = $mysqli->query("SELECT * FROM announcement WHERE tipe = $tipeCourse ORDER BY created_at DESC");
        else $query = $mysqli->query("SELECT * FROM announcement WHERE id_user='$id_user' AND tipe = $tipeCourse ORDER BY created_at DESC");
