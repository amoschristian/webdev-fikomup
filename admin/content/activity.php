...
switch ($show) {
        ...
        //Menampilkan data
    default:
        echo '<h3 class="page-header"><b>Daftar Aktifitas</b>
				<a href="' . $link . '&show=form" class="btn btn-primary btn-sm pull-right top-button">
					<i class="glyphicon glyphicon-plus-sign"></i> Tambah
				</a>
			</h3>';

        buka_tabel(array("Judul Aktifitas", "Tanggal Posting"));
        $no = 1;
        $id_user = $_SESSION['iduser'];

        if ($_SESSION['leveluser'] == "admin") $query = $mysqli->query("SELECT * FROM activity ORDER BY tanggal DESC");
        else $query = $mysqli->query("SELECT * FROM activity WHERE id_user='$id_user' ORDER BY tanggal DESC");
    ...
    case "form":
        if (isset($_GET['id'])) {
            $query = $mysqli->query("SELECT * FROM activity WHERE id='$_GET[id]'");
        ...
        if ($aksi == "Edit" and $_SESSION['leveluser'] != "admin" and $data['id_user'] != $_SESSION['iduser']) {
            ...
            echo '<h3 class="page-header"><b>' . $aksi . ' Aktifitas</b> </h3>';
			buka_form($link, $data['id'], strtolower($aksi));
			buat_textbox("Judul Aktifitas (Bahasa Indonesia)", "judul_terjemahan", $data['judul_terjemahan'], 10);
    ...
    case "action":
        ...
        if ($_POST['aksi'] == "tambah") {
            $mysqli->query("INSERT INTO activity SET
				judul 		    = '$judul',
				judul_seo 	    = '$judul_seo',
            ...
            $mysqli->query("UPDATE activity SET
					judul 		    = '$judul',
					judul_seo 	    = '$judul_seo',
                    judul_terjemahan= '$judul_terjemahan',
					isi			    = '$isi',
    ...
    case "delete":
        ...
        if ($_SESSION['leveluser'] == "admin" or $data['id_user'] == $_SESSION['iduser']) {
            $mysqli->query("DELETE FROM activity WHERE id='$_GET[id]'");
        }
        header('location:' . $link);
        break;
}
