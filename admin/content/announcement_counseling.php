...
switch ($show) {
    ...
    default:
        ...
        buka_tabel(array("Judul Pengumuman", "Kategori", "Tanggal Posting"));
        $no = 1;
        $id_user = $_SESSION['iduser'];

        if ($_SESSION['leveluser'] == "admin") $query = $mysqli->query("SELECT * FROM announcement ORDER BY tanggal DESC");
        else $query = $mysqli->query("SELECT * FROM announcement WHERE id_user='$id_user' ORDER BY tanggal DESC");
        while ($data = $query->fetch_array()) {
            ...
            isi_tabel($no, array($data['judul'], $kat['kategori'], $tanggal), $link, $data['id']);
            $no++;
        }
        tutup_tabel();
