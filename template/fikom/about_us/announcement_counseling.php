...
$currentRecordQuery = "SELECT * FROM announcement WHERE id = $id LIMIT 1;";

if ($id == 'latest') {
    $currentRecordQuery = "SELECT * FROM announcement ORDER BY created_at LIMIT 1;";
}
...
$nextRecordQuery = "SELECT * FROM announcement WHERE id > $id ORDER BY id ASC LIMIT 1;";
$nextRecordResult = $mysqli->query($nextRecordQuery);
if ($nextRecordResult) {
	$nextRecord = $nextRecordResult->fetch_array(MYSQLI_ASSOC);
}

$prevRecordQuery = "SELECT * FROM announcement WHERE id < $id ORDER BY id DESC LIMIT 1;";
